void read_compass() {
  Wire1.beginTransmission(compass_address);                     //Start communication with the compass.
  ...
  Wire1.requestFrom(compass_address, 6);                        //Request 6 bytes from the compass.
  ...
  if (compass_y_horizontal < 0)actual_compass_heading = 180 + (180 + ((atan2(compass_y_horizontal, compass_x_horizontal)) * (180 / 3.14159265)));
  else actual_compass_heading = (atan2(compass_y_horizontal, compass_x_horizontal)) * (180 / 3.14159265);
...
void setup_compass() {
  ...
  Wire1.endTransmission();                                      //End the transmission with the compass.

//Read the calibration values from the EEPROM.
  for (error = 0; error < 6; error ++)compass_cal_values[error] = EEPROM_Read(0x10 + error);
