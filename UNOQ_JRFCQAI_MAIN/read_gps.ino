...
void read_gps(void) {
  ...
  if (new_line_found == 1) {                                                                             //If a new NMEA line is found.
    ...
    if (incomming_message[4] == 'L' && incomming_message[5] == 'L' && incomming_message[7] == ',') {     //When there is no GPS fix or latitude/longitude information available.
      digitalWrite(LED_B, !digitalRead(LED_B));                                      //Change the LED on the STM32 to indicate GPS reception.
  ...
  if (new_gps_data_available) {                                                                           //If there is a new set of GPS data available.
    if (number_used_sats < 8)digitalWrite(LED_B, !digitalRead(LED_B));                //Change the LED on the STM32 to indicate GPS reception.
    else digitalWrite(LED_B, LOW);                                                              //Turn the LED on the STM solid on (LED function is inverted). Check the STM32 schematic.
