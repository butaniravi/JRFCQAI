...
void red_led(int8_t level) {
  digitalWrite(LED_R, !level);    //If a Flip32 is used invert the output.
}
void green_led(int8_t level) {
  digitalWrite(LED_G, !level);    //If a Flip32 is used invert the output.
}

void blue_led(int8_t level) {
  digitalWrite(LED_B, !level);    //If a Flip32 is used invert the output.
}
