 ...
 * * PAY IT FORWARD:
 * * By RAVI C BUTANI - INDIA Contact - ravi.butani03@gmail.com
 * ============================================================================
 */
...
// Compass installation direction: true for forward orientation, false for reverse orientation
...
float pid_p_gain_altitude = 1.4;           //Altitude PID controller P-gain setting (default: 1.4).
float pid_i_gain_altitude = 0.2;           //Altitude PID controller I-gain setting (default: 0.2).
float pid_d_gain_altitude = 1.0;           //Altitude PID controller D-gain setting (default: 0.75).
int pid_max_altitude = 400;                //Maximum output of the PID-controller (+/-).

float gps_p_gain = 2.7;                    //GPS mode PID controller P-gain setting (default: 2.7).
float gps_d_gain = 6.5;                    //GPS mode PID controller D-gain setting (default: 6.5).

//Set magnetic declination (India - Gujarat - Gandhinagar) 
float declination = 0.6;                  //Set the angle between magnetic declination and geographic north

int16_t manual_takeoff_throttle = 1500;       //Input manual hover point when auto-takeoff detection is not needed (between 1400 - 1600)
int16_t motor_idle_speed = 1100;           //Minimum throttle pulse at motor idle (between 1000 - 1200)

uint8_t gyro_address = 0x68;               //I2C address for MPU-6050 gyro
uint8_t MS5611_address = 0x77;             //I2C address for MS5611 barometer
uint8_t compass_address = 0x1E;            //I2C address for QMC5883L electronic compass
uint8_t eeprom_address = 0x50;             //I2C address for external EEPROM

float battery_voltage_calibration = 0.0;   //Battery voltage offset calibration value
float low_battery_warning = 10.5;          //Low battery warning voltage 3.5V * 3 = 10.5V (default = 10.5V).


//Remote parameter tuning configuration:
//Reference videos:
//YouTube: https://youtu.be/ys-YpOaA2ME
#define variable_1_to_adjust battery_compensation    //variable_1_to_adjust Transmitter Channel 1
#define variable_2_to_adjust pid_d_gain_altitude     //variable_2_to_adjust Transmitter Channel 2
#define variable_3_to_adjust dummy_float             //variable_3_to_adjust Transmitter Channel 3
...
void setup() {
  ...
  /**
   * Detect if electronic compass is responding
   */
  ...
  /* Initialize gyroscope and set correct registers */
  ...
  /**
   * Create 5-second delay before calibration
   * 1250 x 4 ms = 5 seconds
   */
  ...
  /* Turn off LED when everything is completed */
   ...
   * Voltage divider (1k and 10k) ratio is 1:11.
   * analogRead => 0=0V ..... 4095=36.3V
   * 36.3/4095 = 112.81.
   * If battery voltage is 10.5V, variable battery_voltage holds 1050
  */
  //Load the battery voltage to the battery_voltage variable.
  ...
  battery_voltage = (float)analogRead(BAT_V) / 112.81;


  /**
   * To calculate pressure, query 6 calibration values from MS5611.
   * These 2-byte values are stored starting from memory location 0xA2
   */ 
  ...
  for (start = 1; start <= 6; start++) {
    ...
    /* Add low byte and high byte to C[x] calibration variable */
...
void mainloop() {
  ...
  if (receiver_watchdog == 750 && start == 2) {
    ...
    */
    ...
    // Remote signal lost failsafe mode
    if (number_used_sats > 5) {
      if (home_point_recorded == 1) {
        channel_5 = 2000;
        channel_7 = 2000;     //Return-to-Home (RTH) mode
      }
  ...
  heading_lock = 0;
  if (channel_6 > 1200)heading_lock = 1;                                           //Channel 6 greater than 1200us enables heading lock mode
  ...
  */
  ...
  if (flight_mode == 3 && channel_7 > 1200) {                                      //If Channel 7 is greater than 1200us and current flight mode is 3, set to RTH mode
    if (waypoint_set == 1 && home_point_recorded == 1 && start == 2) flight_mode = 4;
  }
  ...
  battery_voltage = 1235;//(battery_voltage * 0.92) + ((((float)analogRead(4) / 112.81) + battery_voltage_calibration) * 0.08);
  ...
  analogWrite(M1_ESC,esc_1);                              //Set the throttle receiver input pulse to the ESC 1 output pulse.
  analogWrite(M2_ESC,esc_2);                              //Set the throttle receiver input pulse to the ESC 2 output pulse.
  analogWrite(M3_ESC,esc_3);                              //Set the throttle receiver input pulse to the ESC 3 output pulse.
  analogWrite(M4_ESC,esc_4);                                                         //This will reset timer 4 and the ESC pulses are directly created.
  ...
  if (micros() - loop_timer > 4050)
  {
    ...
    //Serial.println("Loop error");  //debug
  }
