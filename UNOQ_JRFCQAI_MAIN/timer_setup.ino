  ...
  while (Serial1.available() > 0) {
    ...
    if (bufferIndex == 32) {
      ...
      if (calculatedChecksum == transmittedChecksum) {
        ...
        if(channel_9 < 1700) // channel_9 1500 normal 2000 failsafe comment this line if failsafe not needed
        receiver_watchdog=0;
        return true; 
      }
    }
...
void ibus_setup(void) {
  ...
  analogWrite(M1_ESC,1000*819/1000);                              //Set the throttle receiver input pulse to the ESC 1 output pulse.
  analogWrite(M2_ESC,1000*819/1000);                              //Set the throttle receiver input pulse to the ESC 2 output pulse.
  analogWrite(M3_ESC,1000*819/1000);                              //Set the throttle receiver input pulse to the ESC 3 output pulse.
